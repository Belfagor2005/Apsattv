...
exec > >(tee -a /tmp/Apsattv_debug.txt) 2>&1
set -x
...
wget --no-check-certificate 'https://github.com/Belfagor2005/Apsattv/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
cp -r 'Apsattv-main/usr' '/'
set +e
cd
sleep 2
...
IMAGE VERSION: $distro_version" >> /tmp/Apsattv_debug.txt
